...
export function buildSiteThemeCss(theme = {}) {
  const mode = theme.mode === 'dark' ? 'dark' : 'light';
  const accent = /^#[0-9a-f]{6}$/i.test(theme.accent || '') ? theme.accent : '#4da6ff';
  return `
      :root[data-site-theme="light"] .servicesSection { background: linear-gradient(180deg, ${accent}20 0%, #f4f7fa 100%) !important; }
      :root[data-site-theme="light"] .linksSection { background:#fff !important; }
      :root[data-site-theme="dark"] .servicesSection { background: linear-gradient(180deg, ${accent}32 0%, #0b1118 100%) !important; }
      :root[data-site-theme="dark"] .linksSection { background:#0b1118 !important; }
      ${mode === 'dark' ? ':root[data-site-theme="dark"] .servicesSection .sectionIntro span, :root[data-site-theme="dark"] .tileMore, :root[data-site-theme="dark"] .siteFooter a, :root[data-site-theme="dark"] .homeSection a { color: var(--accent-1) !important; }' : ''}
  `;
}
