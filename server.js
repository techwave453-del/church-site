...
async function updateMedia(id, changes) {
  const clean = {
    title: String(changes.title ?? '').trim().slice(0, 200),
    description: String(changes.description ?? '').trim().slice(0, 2000),
    category: String(changes.category ?? 'general').trim().slice(0, 100) || 'general'
  };
  if (!clean.title) throw new Error('Media title is required.');
  if (useSupabase) {
    const { data, error } = await supabase.from('media_items').update(clean).eq('id', Number(id)).select('id,title,type,category,description,url,created_at').maybeSingle();
    if (error) throw error;
    if (!data) return null;
    return data;
  }
  const result = sqlite.prepare('UPDATE media_items SET title=?,description=?,category=? WHERE id=?').run(clean.title, clean.description, clean.category, Number(id));
  if (!result.changes) return null;
  return sqlite.prepare('SELECT id,title,type,category,description,url,created_at FROM media_items WHERE id=?').get(Number(id));
}
...
async function listLiveComments({ admin = false } = {}) {
...
async function addLiveComment(name, comment) {
  if (!useSupabase) throw new Error('Live comments require Supabase to be configured.');
  const cleanName = String(name || '').trim().slice(0, 60);
  const cleanComment = String(comment || '').trim().slice(0, 500);
  if (!cleanName || !cleanComment) throw new Error('Name and comment are required.');
  const { data, error } = await supabase.from('live_comments').insert({ name: cleanName, comment: cleanComment, approved: true }).select('id,name,comment,approved,created_at').single();
  if (error) throw error;
  return data;
}
async function moderateLiveComment(id, approved) {
  if (!useSupabase) throw new Error('Live comments require Supabase to be configured.');
  const { data, error } = await supabase.from('live_comments').update({ approved: Boolean(approved) }).eq('id', Number(id)).select('id,name,comment,approved,created_at').single();
  if (error) throw error;
  return data;
}
async function deleteLiveComment(id) {
  if (!useSupabase) throw new Error('Live comments require Supabase to be configured.');
  const { error } = await supabase.from('live_comments').delete().eq('id', Number(id));
  if (error) throw error;
}
...
app.patch('/api/media/:id',requireSameOrigin,requireAdmin,async(req,res)=>{try{const item=await updateMedia(req.params.id,req.body||{});if(!item)return res.status(404).json({error:'Media item not found.'});res.json(item);}catch(error){console.error(error);res.status(400).json({error:error.message||'Unable to update media.'});}});
