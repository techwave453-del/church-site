...
if (isProduction) {
  ...
  if (!useSupabase) throw new Error('SUPABASE_URL and SUPABASE_SERVICE_ROLE_KEY are required in production.');
}
...
const sqlite = useSupabase && fs.existsSync(path.join(dbDir, 'site.db'))
...
async function ensureSupabaseStorage() {
  ...
  if (created.error && !/already exists/i.test(created.error.message || '')) {
    throw new Error(`Unable to create Supabase storage bucket: ${created.error.message}`);
  }
}
...
async function ensureSupabaseDatabase() {
  ...
  if (error) {
    throw new Error(`Supabase schema is not ready. Run supabase-schema.sql in the Supabase SQL Editor. ${error.message}`);
  }
}
...
async function ensureDefaultSiteContent() {
  if (useSupabase) {
    const rows = Object.entries(DEFAULT_SITE_CONTENT).map(([key, value]) => ({
      key,
...
async function ensureAdminUser() {
  ...
  if (useSupabase) {
    ...
    if (!row) {
      const result = await supabase.from('admin_users').insert({ username, password_hash: hash, role: 'admin' }).select().single();
      if (result.error) throw result.error;
    } else if (!bcrypt.compareSync(password, row.password_hash) || row.role !== 'admin') {
      const result = await supabase.from('admin_users').update({ password_hash: hash, role: 'admin' }).eq('id', row.id);
      if (result.error) throw result.error;
    }
    return;
  }
...
function mimeFromExt(ext) {
  return ({'.jpg':'image/jpeg','.jpeg':'image/jpeg','.png':'image/png','.webp':'image/webp','.gif':'image/gif','.mp4':'video/mp4','.webm':'video/webm','.mov':'video/quicktime','.mp3':'audio/mpeg','.m4a':'audio/mp4','.wav':'audio/wav','.ogg':'audio/ogg','.pdf':'application/pdf'})[ext] || 'application/octet-stream';
}
...
async function migrateSqliteIfPresent() {
  ...
  try {
    ...
    for (const user of users) {
      const existing = await supabase.from('admin_users').select('id').eq('username', user.username).maybeSingle();
      if (existing.error) throw existing.error;
      if (!existing.data) {
        const inserted = await supabase.from('admin_users').insert(user);
    ...
    for (const item of media) {
      ...
      if (item.file_path && fs.existsSync(item.file_path)) {
        const ext = path.extname(item.file_path).toLowerCase() || '.bin';
        storagePath = `migrated/${item.id}-${Date.now()}${ext}`;
...
async function dbGetAdmin(username) {
  if (useSupabase) return (await supabase.from('admin_users').select('*').eq('username', username).maybeSingle()).data;
  return sqlite.prepare('SELECT * FROM admin_users WHERE username=?').get(username);
}
...
async function getSiteContent() {
  if (useSupabase) {
    ...
    if (error) throw error;
    const payload = {}; for (const row of data || []) payload[row.key] = parseStoredValue(row.value);
    return mergeSiteContent(payload);
  }
  const payload = {}; sqlite.prepare('SELECT key,value FROM site_content').all().forEach(({key,value}) => { payload[key] = parseStoredValue(value); });
  return mergeSiteContent(payload);
}

async function saveSiteContent(body) {
  const rows = Object.entries(body || {}).map(([key,value]) => ({ key, value: typeof value === 'string' ? value : JSON.stringify(value) }));
  if (useSupabase) { const { error } = await supabase.from('site_content').upsert(rows, { onConflict: 'key' }); if (error) throw error; return; }
  const update = sqlite.prepare('INSERT INTO site_content (key,value) VALUES (?,?) ON CONFLICT(key) DO UPDATE SET value=excluded.value');
  const tx = sqlite.transaction(() => rows.forEach(row => update.run(row.key,row.value))); tx();
}

async function listMedia() {
  if (useSupabase) { const { data,error } = await supabase.from('media_items').select('id,title,type,category,description,url,created_at').order('created_at',{ascending:false}); if(error) throw error; return data || []; }
  return sqlite.prepare('SELECT id,title,type,category,description,url,created_at FROM media_items ORDER BY created_at DESC').all();
}

async function addMedia(item) {
  if (useSupabase) { const { data,error } = await supabase.from('media_items').insert(item).select('id,title,type,category,description,url,created_at').single(); if(error) throw error; return data; }
  const result = sqlite.prepare('INSERT INTO media_items (title,type,category,description,url,file_path) VALUES (?,?,?,?,?,?)').run(item.title,item.type,item.category,item.description,item.url,item.file_path || null); return {id:result.lastInsertRowid,title:item.title,type:item.type,category:item.category,description:item.description,url:item.url};
}

async function findMedia(id) {
  if (useSupabase) return (await supabase.from('media_items').select('*').eq('id', Number(id)).maybeSingle()).data;
  return sqlite.prepare('SELECT * FROM media_items WHERE id=?').get(Number(id));
}

async function removeMedia(id, item) {
  if (useSupabase) {
    if (item.storage_path) { const removed = await supabase.storage.from(STORAGE_BUCKET).remove([item.storage_path]); if (removed.error) console.error('Storage delete failed:', removed.error); }
    const { error } = await supabase.from('media_items').delete().eq('id', Number(id)); if (error) throw error; return;
  }
...
app.use(session({secret:process.env.SESSION_SECRET||'development-only-session-secret',resave:false,saveUninitialized:false,cookie:{maxAge:1000*60*60*12,httpOnly:true,sameSite:'lax',secure:isProduction}}));
...
const distPath=path.join(__dirname,'dist'),indexPath=path.join(distPath,'index.html'); if(fs.existsSync(distPath))app.use(express.static(distPath));
app.get('/admin',(_req,res)=>res.sendFile(path.join(__dirname,'admin.html'))); app.get(/^\/admin(?:\/.*)?$/,(req,res)=>res.sendFile(path.join(__dirname,'admin.html')));
app.use((req,res,next)=>{if(req.path.startsWith('/api')||req.path.startsWith('/uploads'))return next();res.sendFile(fs.existsSync(indexPath)?indexPath:path.join(__dirname,'index.html'));});
app.use((err,_req,res,_next)=>{if(err instanceof multer.MulterError||err?.message?.startsWith('Unsupported file type'))return res.status(400).json({error:err.message});console.error(err);res.status(500).json({error:'Internal server error.'});});
app.listen(port,'0.0.0.0',()=>console.log(`AIC Kitanga admin server running on http://0.0.0.0:${port} using ${useSupabase?'Supabase':'SQLite'}`));
