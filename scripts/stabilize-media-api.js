...
// when the field is actually supplied, preserving existing values otherwise.
...
const newClean = "const clean={title:String(changes.title??'').trim().slice(0,200),description:String(changes.description??'').trim().slice(0,2000),category:String(changes.category??'general').trim().slice(0,100)||'general'};if(Object.prototype.hasOwnProperty.call(changes,'featured'))clean.featured=Boolean(changes.featured);";
...
console.log('Stabilized Media API to expose and persist the featured flag.');
