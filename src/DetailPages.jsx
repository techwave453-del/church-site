...
import { ArrowLeft, ArrowRight, CalendarDays, Clock3, Heart, MapPin, Phone, Mail, PlayCircle, Users, BookOpen, Gift, MessageCircle } from 'lucide-react';

const details = {
  'sunday-worship-service': {
    eyebrow: 'Worship Service', title: 'Sunday Worship Service',
    summary: 'Gather with the Kingdom Fellowship Christian Church family for worship, the Word of God, prayer and fellowship.',
    schedule: 'Every Sunday · 9:00 AM',
    sections: [
      ['What to Expect', 'A welcoming worship experience with congregational praise, biblical teaching, prayer and opportunities to connect with other believers.'],
      ['Who Can Attend?', 'Everyone is welcome—families, young people, first-time visitors and members of the church family.'],
      ['Come Prepared', 'Bring your Bible, an expectant heart and a friend. Arrive a little early so you have time to settle in and connect.']
    ]
  },
  'healing-and-deliverance-service': {
    eyebrow: 'Prayer & Ministry', title: 'Healing and Deliverance Service',
    summary: 'A focused time of prayer, biblical ministry and seeking God for healing, freedom and restoration.',
    schedule: 'Every Tuesday · 5:30 PM',
    sections: [
      ['A Time of Prayer', 'Come ready to pray and receive ministry in an atmosphere centred on faith in Jesus Christ.'],
      ['Biblical Encouragement', 'The service combines Scripture, prayer and practical encouragement for people trusting God through difficult seasons.'],
      ['Bring Someone Along', 'Invite a friend or family member who would appreciate a supportive, prayerful church community.']
    ]
  },
  'power-communion-service': {
    eyebrow: 'Communion & Fellowship', title: 'Power Communion Service',
    summary: 'A midweek gathering centred on communion, worship, the Word and fellowship.',
    schedule: 'Every Wednesday · 5:30 PM',
    sections: [
      ['Remember Christ', 'Share in communion as a church family and reflect on the work of Jesus Christ.'],
      ['Grow in the Word', 'Receive biblical teaching and encouragement designed to strengthen your walk of faith.'],
      ['Connect', 'Midweek services are a practical opportunity to stay connected and encouraged between Sundays.']
    ]
  },
  'worship-word-wonders-night': {
    eyebrow: 'Friday Gathering', title: 'Worship, Word & Wonders Night',
    summary: 'An evening of worship, teaching and prayer as the church seeks God together.',
    schedule: 'Every Friday · 5:30 PM',
    sections: [
      ['Worship', 'Lift your heart in congregational worship and create space to focus on God.'],
      ['The Word', 'Hear practical, Scripture-based teaching for everyday Christian living.'],
      ['Wonders', 'Join the church in prayer and expectation for God to work in the lives of His people.']
    ]
  },
  'commanding-the-day-midnight-prayer': {
    eyebrow: 'Prayer Watch', title: 'Commanding the Day Midnight Prayer',
    summary: 'A dedicated prayer gathering for believers who want to begin a new season, day or month in prayer.',
    schedule: 'Last Friday of the month · 11:00 PM',
    sections: [
      ['A Night of Prayer', 'Set aside the night to pray, reflect and seek God together with the church family.'],
      ['Spiritual Preparation', 'Use the gathering to commit plans, families, ministries and personal goals to God.'],
      ['Stay Connected', 'Check the church announcements for any changes to the monthly prayer schedule.']
    ]
  },
...
function DetailPage({ church, type, onBack }) {
  ...
  const item = details[key] || details[slugify(key)] || { eyebrow: 'Kingdom Fellowship', title: key, summary: 'More information will be added by the church team.', sections: [['More Information','Please contact the church for the latest details about this programme or ministry.']] };
  return <div className="detailPage">
    <header className="detailHeader"><button className="detailBack" onClick={onBack}><ArrowLeft size={18}/> Back to website</button><div className="detailBrand"><strong>{church?.name || 'Kingdom Fellowship Christian Church'}</strong><span>{church?.tagline || 'Revealing Christ to Nations'}</span></div></header>
    <main>
      <section className="detailHero"><span>{item.eyebrow}</span><h1>{item.title}</h1><p>{item.summary}</p>{item.schedule&&<div className="detailSchedule"><Clock3 size={18}/>{item.schedule}</div>}</section>
      <section className="detailGrid">{item.sections.map(([heading,text])=><article key={heading}><div className="detailIcon"><ArrowRight size={18}/></div><h2>{heading}</h2><p>{text}</p></article>)}</section>
      <section className="detailCta"><div><span>Kingdom Fellowship Christian Church</span><h2>Have questions or want to get connected?</h2><p>Our church team will be happy to help you with directions, registration, programmes or any other enquiry.</p></div><div className="detailActions"><a href={`tel:${church?.phone || ''}`}><Phone size={17}/> Call</a><a href={`mailto:${church?.email || ''}`}><Mail size={17}/> Email</a><button onClick={onBack}><ArrowLeft size={17}/> Return to site</button></div></section>
    </main>
    <footer className="detailFooter"><strong>{church?.name || 'Kingdom Fellowship Christian Church'}</strong><span>{church?.footerTagline || 'Revealing Christ to Nations'}</span></footer>
  </div>;
}
