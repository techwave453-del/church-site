...
export function registerAdminCmsRoutes({ app, supabase, sqlite, requireAdmin, requireSameOrigin, requirePermission }) {
  ...
  async function savePage(input, id = null) {
    ...
    const savedPage = (await listPages(true)).find(item => String(item.id) === String(pageId));
    if (useSupabase) { const result = await supabase.from('cms_page_revisions').insert({ page_id: pageId, snapshot: savedPage }); if (result.error) throw result.error; }
    else sqlite.prepare('INSERT INTO cms_page_revisions (page_id,snapshot) VALUES (?,?)').run(pageId, JSON.stringify(savedPage));
    return savedPage;
  }
  ...
  app.get('/api/cms/pages/:id/revisions', requireAdmin, permission('pages.view'), async (req, res) => { try { const id = Number(req.params.id); if (useSupabase) { const result = await supabase.from('cms_page_revisions').select('id,page_id,created_at,snapshot').eq('page_id', id).order('created_at', { ascending: false }).limit(50); if (result.error) throw result.error; res.json(result.data || []); } else res.json(sqlite.prepare('SELECT id,page_id,created_at,snapshot FROM cms_page_revisions WHERE page_id=? ORDER BY created_at DESC LIMIT 50').all(id).map(item => ({ ...item, snapshot: JSON.parse(item.snapshot) }))); } catch (error) { console.error(error); res.status(500).json({ error: 'Unable to load page revisions.' }); } });
  app.post('/api/cms/pages/:id/revisions/:revisionId/restore', requireSameOrigin, requireAdmin, permission('pages.edit'), async (req, res) => { try { const pageId = Number(req.params.id); const revisionId = Number(req.params.revisionId); let revision; if (useSupabase) { const result = await supabase.from('cms_page_revisions').select('snapshot').eq('id', revisionId).eq('page_id', pageId).maybeSingle(); if (result.error) throw result.error; revision = result.data; } else revision = sqlite.prepare('SELECT snapshot FROM cms_page_revisions WHERE id=? AND page_id=?').get(revisionId, pageId); if (!revision) return res.status(404).json({ error: 'Revision not found.' }); res.json(await savePage(revision.snapshot ? (typeof revision.snapshot === 'string' ? JSON.parse(revision.snapshot) : revision.snapshot) : {}, pageId)); } catch (error) { res.status(400).json({ error: error.message || 'Unable to restore revision.' }); } });
