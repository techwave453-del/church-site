...
function installLoginDesign(){if(typeof window.applyAdminLoginUI==='function')window.applyAdminLoginUI()}
async function loadLoginBranding(){try{const r=await fetch('/api/site/content',{credentials:'include',cache:'no-store'});if(!r.ok)return;const payload=await r.json().catch(()=>null);window.refreshAdminLoginBranding?.(payload)}catch(error){console.debug('Admin login branding unavailable:',error.message)}}
function setAuthenticatedUI(authenticated){if(typeof window.setAdminAuthenticatedUI==='function')window.setAdminAuthenticatedUI(!!authenticated);document.body.classList.toggle('admin-login-page',!authenticated)}
function showSetup(username=''){const value=String(username||'').trim();window.location.assign(value?`/admin/admin-access.html?username=${encodeURIComponent(value)}`:'/admin/admin-access.html')}
...
async function restoreSession(){
  // A browser refresh must not be treated as a logout. The server-side
  // session cookie is the source of truth; ask the API for the current user.
  try{
    const r=await api()('/api/admin/me',{method:'GET',cache:'no-store'});
    if(!r.ok)return false;
    const data=await r.json().catch(()=>({}));
    const user=data.user||data.admin||data;
    if(!user||data.authenticated===false)return false;
    window.__adminExplicitLogin=true;window.__adminAuthenticated=true;window.__adminUser=user;return true;
...
check:async function(){return window.__adminAuthenticated===true},
restore:restoreSession
};
window.login=event=>window.adminSession.login(event);window.logout=()=>window.adminSession.logout();window.checkApi=()=>window.adminSession.check();window.initAdminSession=()=>window.adminSession.check();
window.restoreAdminSession=restoreSession;
})();