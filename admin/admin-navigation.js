...
const GROUPS=[
 {id:'content',label:'Website Content',items:['identity','about','services','links','classes']},
 {id:'media',label:'Media',items:['mediaSettings','media']},
 {id:'live',label:'Live',items:['live','comments']},
 {id:'appearance',label:'Appearance',items:['theme']},
...
function loadStyles(){
 ...
 .admin-section-mode .admin-menu-toggle{display:none;margin-left:auto}
 ...
 @media(max-width:700px){
   .admin-section-mode .admin-navigation{padding:8px 0;margin-bottom:12px;gap:0;box-shadow:0 3px 10px #0000000d}
   .admin-section-mode .admin-menu{display:none}
   .admin-section-mode .admin-menu.mobile-open{display:block;position:absolute;left:0;right:0;top:100%;padding:7px;background:var(--bg);border:1px solid var(--line);border-radius:0 0 12px 12px;box-shadow:0 12px 28px #0003;z-index:31}
   .admin-section-mode .admin-menu.mobile-open .admin-menu-panel{position:static;display:grid;min-width:0;max-width:none;border:0;box-shadow:none;padding:0;background:transparent}
   .admin-section-mode .admin-menu-toggle{display:inline-flex;align-items:center;gap:7px;background:var(--brand);padding:10px 13px}
   .admin-section-mode .admin-menu-toggle::before{content:'☰'}
   .admin-section-mode .admin-mobile-label{display:inline;font-weight:700}
   .admin-section-mode .admin-menu-button{display:none}
   .admin-section-mode .admin-menu.mobile-open .admin-menu-button{display:none}
   .admin-section-mode .admin-menu.mobile-open .admin-menu-toggle{display:inline-flex}
   .admin-section-mode .admin-navigation{position:relative}
   .admin-section-mode .admin-navigation.mobile-menu-open .admin-menu{display:block}
 }
 `;
...
function buildNavigation(){
 ...
 GROUPS.forEach(group=>{
   ...
   group.items.forEach(id=>{
     const item=getView(id);if(!item)return;
     const child=document.createElement('button');child.type='button';child.className='admin-menu-item';child.dataset.view=id;child.dataset.permission=item[2];child.textContent=item[1];child.setAttribute('role','menuitem');
...
function applyPermissions(){
 ...
 GROUPS.forEach(group=>{
   const wrapper=nav.querySelector(`[data-group="${group.id}"]`);if(!wrapper)return;
   let visible=0;
...
function showView(id){
 ...
 nav?.querySelectorAll('.admin-menu-item').forEach(b=>b.classList.toggle('selected',b.dataset.view===id));
 const group=GROUPS.find(g=>g.items.includes(id));
 nav?.querySelectorAll('.admin-menu').forEach(m=>m.classList.toggle('active-menu',m.dataset.group===group?.id));
