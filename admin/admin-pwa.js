(function(){
  ...
  const ADMIN_MANIFEST='/admin-pwa.webmanifest?v=admin-9';
  const ADMIN_WORKER='/admin-service-worker.js?v=admin-7';
  ...
  function addInstallEntry(){
    ...
    if(!b.dataset.pwaClickReady){b.addEventListener('click',install);b.dataset.pwaClickReady='true';}
    setButtonState();
    return b;
  }
  ...
  async function init(){if(initialized)return;initialized=true;addManifest();if(isStandalone()||launchedAsAdminPwa())installed=true;await checkDeploymentVersion();registerAdminWorker();ensure();setTimeout(ensure,250);setTimeout(ensure,1000);setTimeout(ensure,2500);new MutationObserver(ensure).observe(document.body,{childList:true,subtree:true});}
