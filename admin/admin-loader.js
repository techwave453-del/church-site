(function(){
  ...
  let started=false;
  function loadScript(name){return new Promise((resolve,reject)=>{if(document.querySelector(`script[data-admin-module="${name}"]`))return resolve();const s=document.createElement('script');s.src='/admin/'+name;s.dataset.adminModule=name;s.onload=resolve;s.onerror=()=>reject(new Error('Failed to load '+name));document.head.appendChild(s);});}
  window.loadAdminModules=async function(){
    ...
    try{
      ...
      if(!authenticated){started=false;return false;}
      ...
      return true;
    }catch(error){console.error('Admin modules failed to initialize:',error);started=false;return false;}
  };
})();