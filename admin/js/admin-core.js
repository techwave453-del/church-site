import { api, checkApiStatus, getSession } from './auth.js';
...
async function load() {
  try {
    ...
    textKeys.forEach(key => { if ($(key)) $(key).value = content[key] ?? ''; });
...
async function loginForm(event) { event.preventDefault(); try { const response = await (await import('./auth.js')).login($('username').value, $('password').value); if (response.ok) { $('login').classList.add('hidden'); $('app').classList.remove('hidden'); msg('Signed in successfully.'); await load(); } else $('loginMsg').textContent = (await response.json()).error || 'Invalid username or password.'; } catch (error) { $('loginMsg').textContent = error.message || 'Unable to sign in. Please try again.'; } }

Object.assign(window, { api, login: loginForm, tab, load, save, upload, loadMedia, loadComments, addItem });
...
async function init() {
  ...
  } catch { msg('Unable to connect to the admin session. Please refresh and try again.', true); }
}
init();
