(() => {
  ...
  style.textContent = `
    ...
    .church-chat-welcome-bubble:before{
      content:'';position:absolute;left:0;top:0;width:4px;height:100%;
      background:linear-gradient(180deg,var(--accent-1,#d9ecff),var(--accent-2,#4da6ff),var(--accent-3,#24537d));
    }
    .church-chat-welcome-bubble:after{
      content:'';position:absolute;right:-1px;bottom:-1px;width:18px;height:18px;
      background:var(--chat-welcome-bg,#fff);clip-path:polygon(0 0,100% 100%,0 100%);transform:translateY(1px);
    }
    .church-chat-welcome-top{display:flex;align-items:center;gap:10px;padding-right:24px;margin-bottom:12px}
    .church-chat-welcome-avatar{
      position:relative;width:42px;height:42px;flex:0 0 42px;border-radius:50%;display:grid;place-items:center;
      background:linear-gradient(145deg,var(--accent-1,#d9ecff),var(--accent-2,#4da6ff));
      color:#fff;font-size:18px;font-weight:800;box-shadow:0 4px 12px var(--accent-shadow,rgba(77,166,255,.24));
    ...
    .church-chat-welcome-close{
      position:absolute;right:8px;top:8px;width:25px;height:25px;border:0;border-radius:50%;
      background:rgba(0,0,0,.045);color:#71808c;font-size:17px;line-height:1;cursor:pointer;z-index:4;
    }
    ...
    .church-chat-welcome-cta{
      display:inline-flex;align-items:center;justify-content:center;gap:6px;padding:8px 13px;border-radius:999px;
      background:linear-gradient(135deg,var(--accent-2,#4da6ff),var(--accent-3,#24537d));color:#fff;
      font-size:10.5px;font-weight:800;box-shadow:0 6px 14px var(--accent-shadow,rgba(77,166,255,.23));
    }
    ...
    @media(max-width:600px){
      .church-chat-welcome{right:10px!important;left:10px!important;bottom:calc(116px + env(safe-area-inset-bottom))!important;width:auto!important;max-width:none!important}
      .church-chat-welcome-bubble{border-radius:18px 18px 6px 18px;padding:14px 14px 13px}
      .church-chat-welcome-avatar{width:38px;height:38px;flex-basis:38px}.church-chat-welcome-heading{font-size:14px}.church-chat-welcome-text{font-size:12px}.church-chat-welcome-actions{margin-top:11px}
    }
  ...
  function resolveLogo(value){
    ...
    if(typeof value==='object'){
      for(const key of ['url','src','publicUrl','public_url','logoUrl','logo_url','fileUrl','file_url','href','path','logo','image','thumbnail','value']){
        const found=resolveLogo(value[key]);if(found)return found;
      }
    }
    return '';
  }
  ...
  async function showPopup(){
    ...
    popup.setAttribute('aria-label','Church chat invitation');
    popup.innerHTML=`
      <div class="church-chat-welcome-bubble">
        <button class="church-chat-welcome-close" type="button" aria-label="Close chat invitation">×</button>
        <div class="church-chat-welcome-top">
          <div class="church-chat-welcome-avatar">${logo?`<img src="${escapeHtml(logo)}" alt="">`:'✦'}<span class="church-chat-welcome-online"></span></div>
          <div><div class="church-chat-welcome-name">Church Team</div><div class="church-chat-welcome-role">Online · Ready to help</div></div>
          <span class="church-chat-welcome-time">now</span>
        </div>
        <div class="church-chat-welcome-heading">${message.heading}</div>
        <div class="church-chat-welcome-text">${message.text}</div>
        <div class="church-chat-welcome-typing" aria-hidden="true"><i></i><i></i><i></i></div>
        <div class="church-chat-welcome-actions"><span class="church-chat-welcome-cta">${message.cta} <span>›</span></span><span class="church-chat-welcome-later">or close for now</span></div>
      </div>`;

    document.body.appendChild(popup);
    popup.addEventListener('click',e=>{
      if(e.target.closest('.church-chat-welcome-close'))return;
      openChat();removePopup(popup);
    });
    popup.querySelector('.church-chat-welcome-close').addEventListener('click',e=>{
      e.stopPropagation();hiddenByUser=true;sessionStorage.setItem(doneKey(),'done');removePopup(popup);
    });
  ...
  function scheduleRepeat(){
    if(shown<MAX_SHOWS&&!hiddenByUser){
      setTimeout(()=>{showPopup();scheduleRepeat();},REPEAT_DELAY_MS);
    }
  }
