(function(){'use strict';
  ...
  const resolve=(value,depth=0)=>{if(depth>5||value==null)return '';if(Array.isArray(value)){for(const item of value){const found=resolve(item,depth+1);if(found)return found;}return '';}if(typeof value==='object'){const keys=['url','src','publicUrl','public_url','logoUrl','logo_url','fileUrl','file_url','href','path','logo','image','value'];for(const key of keys){const found=resolve(value[key],depth+1);if(found)return found;}return '';}const raw=String(value).trim();if(!raw)return '';if(/^data:image\\//i.test(raw)||/^blob:/i.test(raw))return raw;try{const u=new URL(raw,location.origin);if(['http:','https:'].includes(u.protocol))return u.href}catch(_){}return '';};
  const mediaUrl=(item)=>resolve(item?.url||item?.publicUrl||item?.public_url||item?.fileUrl||item?.file_url||item?.path||item?.src||item?.image||item?.thumbnail||'');
