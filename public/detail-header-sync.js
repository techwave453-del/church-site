(function(){
  ...
  async function install(){
    ...
    let logoUrl='';
    try{const r=await fetch('/api/site/content');if(r.ok){const data=await r.json();logoUrl=data?.logo||''}}catch(_e){}
    if(!document.body.contains(old))return;
    const header=document.createElement('header');header.className='detailSiteHeader';
    const identity=document.createElement('button');identity.className='brand';identity.type='button';identity.setAttribute('aria-label',name+' home');identity.onclick=()=>{window.location.href='/#home'};
    ...
    Object.keys(labels).forEach(label=>{const a=document.createElement('a');a.textContent=labels[label];a.href=label==='Home'?'/#home':'/#detail/'+routes[label];nav.appendChild(a)});
